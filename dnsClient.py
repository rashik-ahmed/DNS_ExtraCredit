...
def dns_query(type, name, server):
    ...
    server_address = (server, 53)  # Port number for DNS is 53
    ...
    qname_parts = name.split('.')  # Split domain name into parts
    ...
    for _ in range(ANCOUNT):
        ...
        type, cls, ttl, rdlength = struct.unpack('!HHIH', response_answer[offset:offset+10])  # What is the offset value in bytes? (10 bytes)
